...
def compute_loss(p, targets, model, prunable_modules=None):  # predictions, targets, model
    ...
    for i, pi in enumerate(p):  # layer index, layer predictions
        ...
        if nb:
            ...
            if h["l1"]:
                ...
                lbox += nn.SmoothL1Loss(reduction=red)(pbox.t(), tbox[i].t())
            else:
...
def plot_labels(labels, label_path="labels.png"):
    # plot dataset labels
    ...
    plt.savefig(label_path, dpi=200)
...
def plot_results(start=0, stop=0, bucket='', id=(), labels=()):  # from utils.utils import *; plot_results()
    ...
    fig.tight_layout()
    ax[1].legend()
    fig.savefig(os.path.join("outputs", 'results.png'), dpi=200)
