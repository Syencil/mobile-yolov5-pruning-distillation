...
def parse_model(md, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(md['backbone'] + md['head']):  # from, number, module, args
        ...
        if m in [nn.Conv2d, Conv, Bottleneck, SPP, DWConv, MixConv2d, Focus, ConvPlus, BottleneckCSP, MobileBottleneck, InvertedResidual, ConvBNReLU]:
            c1, c2 = ch[f], args[0]
...
if __name__ == '__main__':
    ...
    # prunable_module_type = (nn.BatchNorm2d, nn.Conv2d)
    # for i, m in enumerate(model.modules()):
    #     print(i, isinstance(m, prunable_module_type),  "--->", m)
